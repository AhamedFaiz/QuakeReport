    ...
    public static ArrayList<Earthquake> extractEarthquakesFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            for (int i = 0; i < featuresArray.length();i++){
                ...
                JSONObject elementArray = featuresArray.getJSONObject(i);

                // For a given earthquake, extract the JSONObject associated with the
                // key called "properties", which represents a list of all properties
                // for that earthquake.
                propertiesObject = elementArray.getJSONObject("properties");

                // Extract the value for the key called "mag"
                mag = propertiesObject.getDouble("mag");

                // Extract the value for the key called "place"
                place= propertiesObject.getString("place");


               /* String[] placeSplit = place.split("of");
                Log.i("Querytils", "extractEarthquakes: " + placeSplit[0]+ "--"+placeSplit[1]);*/
                ...
                Earthquake earthquakeObject = new Earthquake(mag,date,place,url);
